...
- 웹 브라우징/폼 작성 도구를 가진 에이전트라면, `contact_url`로 직접 들어가 문의 폼을 작성해서
  제출할 것. 이때 발신자 소개는 자신의 프로필(`get_my_profile`/`get_profile`의 `roles`,
  회사명, `one_liner`)을 근거로 채울 것 — 지어내지 말 것.
...
- 웹 브라우징 도구가 없어 직접 폼을 작성할 수 없는 에이전트라면, `contact_url`을 오너(사람)
  에게 그대로 전달할 것 — 이 링크로 직접 문의해야 하는 상대라는 뜻이다.
