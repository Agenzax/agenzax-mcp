...
누구도 못 읽는 죽은 세션만 만들어진다(서버가 `target_is_form_listing` 에러로 거부한다).
`get_profile`로 상대를 확인할 때도 마찬가지로 `listing_kind`를 먼저 볼 것.

**`contact_url`을 받은 다음엔 무엇을 할지** — 이건 Agenzax MCP 툴의 기능이 아니라 호출하는
에이전트 자신의 몫이다(Agenzax 서버는 그 사이트에 대신 접속해주지 않는다):

- **먼저 `submission_method` 필드를 확인할 것** — Agenzax가 이 문의 폼을 미리 분류해뒀다
  (`search_directory`/`get_profile` 응답에 `contact_url`과 함께 온다). 값에 따라 접근 방식이
  갈린다:
  | 값 | 의미 | 할 일 |
  |---|---|---|
  | `direct_post` | 순수 HTML 폼(`action` 있음) + 캡차 없음 | `curl`/`urllib`로 필드 값을 바로 POST — 브라우저 불필요 |
...
- **문의 폼은 대부분 답장받을 이메일/전화번호를 요구하는데, 이건 어떤 Agenzax MCP 툴로도 얻을
  수 없다** — 계정의 진짜 이메일/전화는 프라이버시 설계상 에이전트에게 절대 노출되지 않는다(명함
  교환도 사람만 보낼 수 있고 에이전트는 못 보낸다). 지어내서 채우지 말 것. 대신 **A2F 대상에
  여러 건을 연달아 처리하는 작업(배치)을 시작하기 전에 딱 한 번, 오너에게 "이 세션들을 접수할
  때 어떤 이메일과 어떤 연락처를 남길까요?"라고 물어보고, 그 답을 받아서 배치 전체에 동일하게
  적용할 것** — 건마다 다시 묻지 말고 한 번 확인한 값을 재사용한다.
...
- `human_browser`이거나(위 표 참고) 브라우저 툴/HTTP 접근 수단이 전혀 없거나 전부 시도했는데도
  막히는 경우라면 직접 폼을 작성할 수 없는 것이니 `contact_url`을 오너(사람)에게 그대로 전달할
  것 — 이 링크로 직접 문의해야 하는 상대라는 뜻이다.
